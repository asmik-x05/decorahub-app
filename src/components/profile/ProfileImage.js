...
const ProfileImage = () => {
  ...
  return (
    <div className="flex items-center pt-2 pb-10 gap-5">
      {user?.profileImgUrl || localUrl ? (
        <Image
          src={localUrl || user.profileImgUrl}
          height={200}
          width={200}
          alt={user.Name}
      ...
      <button
        onClick={updateImage}
        className="inline-flex gap-2 items-center px-5 py-2.5 text-sm font-medium text-center text-white bg-primary rounded-lg focus:ring-4 focus:ring-primary/20 dark:focus:ring-primary hover:bg-blue-800"
      >
        Update Image
        {loading && "..."}
      </button>
